    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function checkUserActivation($username)
    {
        if ($user = $this->getUser($username)) {
            return $this->checkActivation($username);
        }
    ...
    {
        $stmt = $this->db->prepare($sql = sprintf('SELECT TOP 1 COUNT(username) AS id FROM %s WHERE Active=1 AND username = :username', $this->config['user_table']));
    ...
    protected function hashPassword($password)
    {
        ...
        return md5('830afea5006de36e41eddb4b33182483'.$password);
    }
    ...
    public function setUser($username, $password, $firstName = null, $lastName = null, $ipAddress, $createdOn, $MemberCardTMP, $GroupId, $CompanyId, $Phone, $Active , $ByOmni)
    {
        ...
        if ($this->getUser($username)) {
            ...
            if($CompanyId) $fields[] = 'CompanyId';
            if($Phone) $fields[] = 'Phone';
            if($Active) $fields[] = 'Active';
            if($ByOmni) $fields[] = 'ByOmni';
        } else {
            $stmt = $this->db->prepare(sprintf('INSERT INTO %s (username, password, first_name, last_name, IpAddress, Email, CreatedOn, MemberCardTMP, GroupId, CompanyId, Phone, Active , ByOmni) VALUES (:username, :password, :firstName, :lastName, :ipAddress, :email, :createdOn, :MemberCardTMP, :GroupId, :CompanyId, :Phone, :Active , :ByOmni)', $this->config['user_table']));
        }

        return $stmt->execute(($this->getUser($username)?compact('username', 'password', 'firstName', 'lastName', 'ipAddress', $fields):compact('username', 'password', 'firstName', 'lastName', 'ipAddress', 'email', 'createdOn', 'MemberCardTMP', 'GroupId', 'CompanyId', 'Phone', 'Active' , 'ByOmni')));
    }
